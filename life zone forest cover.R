...
# Load Helmer age map
age <- raster("/Users/au529793/Projects/Puerto Rico/GIS Data/PR_Forest_Age/pr_forestage_zone_v2/iitf_jgr113_puertorico_forestage_zone_reprojectedWGS84.tif")

### READ IN DATA
r51 <- raster("/Users/au529793/Downloads/prchange_landcov_51to2000/puerto51_sub1_100905_landcov_final.img")

r77 <- raster("/Users/au529793/Downloads/prchange_landcov_51to2000/puerto77_sub1_100905_landcov_urbveg_final.img")

r91 <- raster("/Users/au529793/Downloads/prchange_landcov_51to2000/pr91_100805_final_quarry_recode2landcov_subset.img")

r00 <- raster("/Users/au529793/Downloads/prchange_landcov_51to2000/pr2000_100805_final_quarry_recode2landcov_subset.img")
...
f <- stack(f51, f77, f91, f00)


## USING RASTER
### READ LIFE ZONE AND RASTERIZE
lz <- readOGR("/Users/au529793/Projects/Puerto Rico/GIS Data/PR_Lifezone/Lifezones/lifezones_Project.shp")
...
legend("left", legend=rownames(result_raster), col=1:6, lty=1, bty='n', cex=.75, lwd=3)



### USING SHAPEFILES
# Get results using the polygons to mask different areas
result <- matrix(nrow=length(unique(lz2$ECOZONE)), ncol=4)
rownames(result) <- unique(lz2$ECOZONE)
colnames(result) <- c(1951, 1977, 1991, 2000)

for(i in 1:length(unique(lz2$ECOZONE))){
  focal_lz <- unique(lz2$ECOZONE)[i]
  tmp <- mask(f, lz2[lz2$ECOZONE == focal_lz,])
